...
```{r}
ggplot(metadata, aes(x = Outcome, fill = DeliveryMode)) + geom_bar(position = "dodge") + geom_text(aes(label = after_stat(count)), stat = 'count', position = position_dodge(0.9), vjust = -0.2) + labs(x = "Outcome", y = "Count", fill = "Delivery") + scale_fill_viridis(discrete = TRUE)
```
...
metadata %>%
  ...
  labs(x = "Outcome", y = "BMI (kg/m^2)", fill = "Outcome")
```
...
*A note on statistics: scientists will often define a hypothesis before conducting their research. They will also use statistical tests to test this hypothesis. This is a formal way for us to test if a difference we see is likely to be a real difference or just down to chance.*
